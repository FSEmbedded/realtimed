...
struct srtm_pwm_service
{
    struct _srtm_service service;
    struct srtm_pwm_adapter *adapter;
};

static srtm_status_t SRTM_PwmService_Request(srtm_service_t service, srtm_request_t request)
{
    srtm_status_t status;
    struct srtm_pwm_service *handle = (struct srtm_pwm_service *)(void *)service;
    struct srtm_pwm_adapter *adapter = handle->adapter;
    struct pwm_adapter *pwm_adapter = adapter->pwm_adapter;
    ...
    pwmReq     = (void *)SRTM_CommMessage_GetPayload(request);
...
{
    struct srtm_pwm_service *handle;

    SRTM_DEBUG_MESSAGE(SRTM_DEBUG_VERBOSE_INFO, "%s\r\n", __func__);

    handle = SRTM_Heap_Malloc(sizeof(struct srtm_pwm_service));
    assert(handle);
    if(!handle)
    {
        return SRTM_Status_OutOfMemory;
